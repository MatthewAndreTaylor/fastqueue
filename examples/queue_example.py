...
for q in queue_types:
    ...
    assert [dequeue(queue_structures[q][0]) for _ in range(queue_size)] == list(
        range(queue_size)
    )
    queue_times.append(time.time() - start)
queue_types, queue_times = zip(
    *sorted(zip(queue_types, queue_times), key=lambda x: x[1], reverse=True)
)
